...
* `gamma_s`  有症狀(I_sym)不住院者至康復(R)速率=1/不住院且會康復者在有症狀(I_sym)的時間
* `f_h`  住院者(H)死亡機率
...
* `I_sym`  有症狀感染者
* `H`  所有住院者(接受治療且會死亡)
* `H_mild` 病情較輕之住院者(接受治療且不會死亡)(不是每種模擬情況皆會使用到，目前僅有`progress_02`會用到)
* `F`  死亡者
* `R`  康復者(具有免疫力)
* `Q_E`  被隔離之病情處於潛伏期者
...
def runSEIRS(S, E, I_pre, I_asym, I_sym, H, H_mild, F, R, Q_E, Q_I_pre, Q_I_asym, Q_I_sym, Q_R, N, dt, params, progress_func):
    globals().update(params)  # set values of the simulation parameters to custom values

    E, F, H, H_mild, I_asym, I_pre, I_sym, Q_E, Q_I_asym, Q_I_pre, Q_I_sym, Q_R, R, S = progress_func(E, F, H, H_mild, I_asym, I_pre, I_sym,
    ...
    S, E, I_pre, I_asym, I_sym, H, H_mild, F, R, Q_E, Q_I_pre, Q_I_asym, Q_I_sym, Q_R, N = map(fix,
                                                                                       [S, E, I_pre, I_asym, I_sym, H, H_mild,
                                                                                        F, R, Q_E, Q_I_pre, Q_I_asym,
                                                                                        Q_I_sym, Q_R, N])

    N = sum((S, E, I_pre, I_asym, I_sym, H, H_mild, F, R, Q_E, Q_I_pre, Q_I_asym, Q_I_sym, Q_R))
    return S, E, I_pre, I_asym, I_sym, H, H_mild, F, R, Q_E, Q_I_pre, Q_I_asym, Q_I_sym, Q_R, N
...
class Progresses:
    ...
    '''
    @staticmethod
    def progress_00(E, F, H, H_mild, I_asym, I_pre, I_sym, N, Q_E, Q_I_asym, Q_I_pre, Q_I_sym, Q_R, R, S, dt, params):
        """
        原始模擬方式
        ...
        return E, F, H, H_mild, I_asym, I_pre, I_sym, Q_E, Q_I_asym, Q_I_pre, Q_I_sym, Q_R, R, S

    @staticmethod
    def progress_01(E, F, H, H_mild, I_asym, I_pre, I_sym, N, Q_E, Q_I_asym, Q_I_pre, Q_I_sym, Q_R, R, S, dt, params):
        '''
        假設醫療資源(床位)有限
    ...
    @staticmethod
    def progress_02(E, F, H, H_mild, I_asym, I_pre, I_sym, N, Q_E, Q_I_asym, Q_I_pre, Q_I_sym, Q_R, R, S, dt, params):
        '''
        假設醫療資源(床位)有限
        在部分醫院中的病人(H)轉移至死亡(F)或康復(R)區後，醫院會接收病患(不分病情輕重)至額滿為止，而接收之各類別病患人數由各類別人數之比例而定
        輕症或無症狀者不會死亡，僅有有症狀者會死亡
        '''
        globals().update(params)  # set values of the simulation parameters to custom values
        ...
        H += (I_sym2H + Q_I_sym2H)
        H_mild += (Q_I_asym2H + Q_I_pre2H + Q_E2H)
        I_sym -= I_sym2H
        Q_I_sym -= Q_I_sym2H
        Q_I_asym -= Q_I_asym2H
        Q_I_pre -= Q_I_pre2H
        ...
        Q_I_asym += (d_I_asym2Q_I_asym + d_Q_I_pre2Q_I_asym_dt - d_Q_I_asym2Q_R_dt) * dt
        Q_I_sym += (d_I_sym2Q_I_sym + d_Q_I_pre2Q_I_sym_dt - d_Q_I_sym2Q_R_dt - d_Q_I_sym2F_dt) * dt
        F += (d_H2F_dt + d_H_mild2F_dt + d_I_sym2F_dt + d_Q_I_sym2F_dt) * dt
        Q_R += (d_Q_I_asym2Q_R_dt + d_Q_I_sym2Q_R_dt - d_Q_R2R_dt) * dt
        R += (d_I_asym2R_dt + d_I_sym2R_dt + d_H2R_dt +d_H_mild2R_dt + d_Q_R2R_dt - d_R2S_dt) * dt
...
def simulateSEIRS(S=S, E=E, I_pre=I_pre, I_asym=I_asym, I_sym=I_sym, H=H, H_mild=H_mild, F=F, R=R, Q_E=Q_E, Q_I_pre=Q_I_pre,
